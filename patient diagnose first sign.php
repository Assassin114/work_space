...
<div id="container" class="container">
	...
	<ul>
        ...
        <ul>
        <li  style="background-color:#8C8C8C;" class="fontpx"><a style="color: #000000;" href="patient diagnose first sign.php">各项体征</a></li>
        <li class="fontpx"><a href="patient diagnose first result.php">诊断结果</a></li>
	...
	<form action="patient diagnose first sign.php" method="post">
	<br>
...
<?php
	if(isset($_POST["submit"]) && $_POST["submit"] == "保存并提交")
	{
		...
		else
		{
			...
			$sql = mysql_query("update first_diagnosis set heat='$heat', breath='$breath', low='$low', high='$high', heartrate='$heartrate', color='$color', thick='$thick', wet='$wet', yesorno='$yesorno', allorpart='$allorpart', fall='$fall', location='$location', len='$len', zhishu='$zhishu', jielv='$jielv', maiguan='$maiguan', maifu='$maifu', cun='$cun', guan='$guan', chi='$chi' where username='$username' and number = '$number'");
			//
			if(sql)
			{
				echo("<script>alert('提交成功！');window.location.href='patient diagnose first result.php';</script>");
			}
			else
			{
