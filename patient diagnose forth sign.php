...
<div id="container" class="container">
	...
	<ul>
        ...
        <ul>
        <li style="background-color:#8C8C8C;" class="fontpx"><a style="color: #000000" href="patient diagnose forth sign.php">各项体征</a></li>
	...
	<form action="patient diagnose forth sign.php" method="post">
	<br>
...
<?php
	if(isset($_POST["submit"]) && $_POST["submit"] == "保存并提交")
	{
		...
		else
		{
			...
			$sql = mysql_query("update forth_diagnosis set heat='$heat', breath='$breath', low='$low', high='$high', heartrate='$heartrate', color='$color', thick='$thick', wet='$wet', yesorno='$yesorno', allorpart='$allorpart', fall='$fall', location='$location', len='$len', zhishu='$zhishu', jielv='$jielv', maiguan='$maiguan', maifu='$maifu', cun='$cun', guan='$guan', chi='$chi' where username='$username' and number = '$_SESSION[time]'");
			//
			if(sql)
			{
				echo("<script>alert('提交成功！');window.location.href='patient diagnose forth result.php';</script>");
			}
			else
			{
